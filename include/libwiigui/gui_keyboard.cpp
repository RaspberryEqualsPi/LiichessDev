...
// minimally modified by 26gy2 to fix some problems I was running into
// (fixed scrolling and spacebar)
...
static char * GetDisplayText(char * t)
{
	...
	if (len < 32){
		...
		bufPtr = t + (strlen(t) - MAX_KEYBOARD_DISPLAY + 1); // so that it can scroll
	}
	snprintf(tmptxt, MAX_KEYBOARD_DISPLAY, "%s", bufPtr);
	return &tmptxt[0];
}
